...
N = int(input("Введите длину списки чисел: "))
X = int(input("Введите число, которое мы ищем: "))
A = [i for i in range(1, N + 1)]
print(f'Число {X} повторяется в списки {A.count(X)} раз')
...
X = int(input("Введите число, которое мы ищем: "))
A = [i for i in range(1, N + 1)]
print(min(A, key=lambda i: abs(i-X) if abs(i-X) != 0 else i+X))
...
priceLetters = {1: "AEIOULNSTRАВЕИНОРСТ",
                2: "DGДКЛМПУ",
                3: "BCMPБГЁЬЯ",
                4: "FHVWYЙЫ",
                5: "KЖЗХЦЧ",
...
price = 0
for i in word:
    for k, v in priceLetters.items():
        if i in v:
            price += k
print(f"Цена слова {price}")